<nav class="py-[40px] relative md:block hidden lg:px-0 px-10">
    ...
    <div class="flex gap-x-10 float-right absolute top-1/2 -translate-y-1/2 right-5 lg:right-0 text-lg font-semibold text-white items-center">
        ...
        <a href="{{route('home')}}/dashboard/botUpload">
            <button class="w-[7rem] bg-accent/70 rounded text-white font-bold py-3 px-1 -mt-1">Add Bot</button>
        </a>
        <a href="{{route('home')}}/dashboard/serverUpload"> 
            <button class="w-[7rem] bg-accent/70 rounded text-white font-bold py-3 px-1 -mt-1">Add Server</button>
        </a>
        @if(Auth::check()) 
            <a href="{{route('login')}}">
                <button class="w-[8rem] bg-accent rounded text-white font-semibold py-3 px-4 -mt-1 " >Dashboard</button>
            </a>
        @else 
            <a href="{{route('login')}}">
